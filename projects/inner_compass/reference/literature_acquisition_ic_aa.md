...
- "Lokaler Bestand `Downloads/Literatur`: Zaehlung und CSV-Export **per PowerShell** (`Get-ChildItem -Recurse -File` + `Export-Csv` UTF-8). `pathlib.Path.rglob` allein kann auf Windows bei langen/Unicode-Pfaden weniger Dateien liefern als der Explorer — nicht als SoT fuer die 201er-Zahl verwenden."
